...
export default function ProductGeneraciones({ producto, generaciones: generacionesInit, onBack, onGenerarNuevo, onEliminar }) {
  ...
  // Edit state — keyed by generacion.id
  ...
  function handleEditDone(id, newImagenUrl, newGeminiPrompt, newVersionHistory) {
    setGeneraciones(prev => prev.map(g =>
      g.id === id
        ? { ...g, imagen_url: newImagenUrl, gemini_prompt: newGeminiPrompt, version_history: newVersionHistory }
        : g
    ))
    // Also update fullscreen if it's the same image
  ...
  function handleRestoreVersion(genId, version) {
    setGeneraciones(prev => prev.map(g =>
      g.id === genId
        ? { ...g, imagen_url: version.imagen_url, gemini_prompt: version.gemini_prompt }
        : g
    ))
  ...
  // Rebuild grupos after state update
                ...
                return (
                  <section key={sec} ref={el => { sectionRefs.current[sec] = el }}>
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <span className={`text-xs font-semibold px-3 py-1 rounded-full border ${col.badge}`}>{sec}</span>
                      <div className="flex-1 h-px bg-gray-800" />
                    </div>
    ...
    >
      <div className="relative max-w-sm w-full" onClick={e => e.stopPropagation()}>
        ...
        <div className="mt-3 flex gap-2 justify-center">
          <button
            onClick={onEdit}
            className="flex items-center gap-1.5 text-xs text-violet-400 hover:text-violet-300 border border-violet-800/40 hover:border-violet-700 px-3 py-1.5 rounded-lg transition-all"
          >
...
function GenCard({
  ...
  return (
    <div className={`group relative bg-gray-900 border rounded-xl overflow-hidden transition-colors ${
      isEditing ? 'border-violet-700 col-span-2 sm:col-span-2' : 'border-gray-800 hover:border-gray-700'
    }`}>
      {/* Thumbnail */}
      <div
        className="aspect-[9/16] overflow-hidden cursor-pointer relative"
      ...
      {confirming ? (
        ...
        <div className="border-t border-gray-800 p-2 flex items-center justify-around">
          ...
          <button onClick={onDownload} title="Descargar"
            className="p-1.5 text-gray-500 hover:text-white rounded-lg hover:bg-gray-800 transition-all">
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
            </svg>
          ...
          {!isEditing && (
            <button onClick={onConfirmDelete} title="Eliminar"
...
function EditPanel({ genId, versiones, onCancel, onDone, onRestoreVersion }) {
  ...
  const [refImages, setRefImages] = useState([]) // [{data, mimeType, preview}]
  ...
  function handleAddRefImages(e) {
    ...
    files.slice(0, restante).forEach(file => {
      ...
      reader.onload = ev => {
        ...
        setRefImages(prev => [
          ...prev,
          { data, mimeType, preview: dataUrl, name: file.name },
        ])
      }
  ...
  async function handleRegenerar() {
    ...
    try {
      const res = await fetch(`/api/generaciones/${genId}/edit`, {
        ...
        body: JSON.stringify({
          userEdits: texto.trim(),
          referenceImages: refImages.map(({ data, mimeType }) => ({ data, mimeType })),
        }),
      })
      clearTimeout(t)
  ...
  return (
    <div className="border-t border-violet-900/40 bg-gray-950 p-3 flex flex-col gap-3">
      ...
      <textarea
        ...
        disabled={cargando}
        rows={3}
        placeholder="Ej: cambiar zapatillas del jugador a blancas, hacer el headline más grande, fondo más oscuro..."
            ...
            {refImages.map((img, idx) => (
              <div key={idx} className="relative group/ref">
                ...
                <button
                  ...
                  className="absolute -top-1 -right-1 w-4 h-4 bg-gray-800 hover:bg-red-700 rounded-full text-gray-400 hover:text-white text-[10px] flex items-center justify-center opacity-0 group-hover/ref:opacity-100 transition-all"
                >
                  ×
                </button>
              </div>
          ...
          {PASOS_EDIT.map((paso, idx) => (
            <div key={idx} className={`flex items-center gap-2 transition-all duration-500 ${
              ...
              <div className={`w-3.5 h-3.5 rounded-full flex-shrink-0 flex items-center justify-center ${
                idx < pasoActual ? 'bg-green-500/20 border border-green-500/40' :
                idx === pasoActual ? 'border border-violet-400/40' : 'border border-gray-700'
              }`}>
                {idx < pasoActual ? (
      ...
      <div className="flex gap-2">
        <button
          onClick={onCancel}
          disabled={cargando}
          className="flex-1 text-[11px] text-gray-500 hover:text-gray-300 border border-gray-700 hover:border-gray-600 py-1.5 rounded-lg transition-all disabled:opacity-40"
        >
          Cancelar
        </button>
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1 text-[11px] text-white bg-violet-600 hover:bg-violet-500 disabled:opacity-40 disabled:cursor-not-allowed py-1.5 rounded-lg transition-all font-medium"
        >
          {cargando ? (
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-1">
            {versiones.map((v, idx) => (
              <button
                ...
                disabled={cargando}
                title={`v${versiones.length - idx} — ${new Date(v.timestamp).toLocaleDateString('es-AR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}`}
