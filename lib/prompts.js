...
export const PROMPT_ANALYZE_TEMPLATE_SYSTEM = `Analyze this ad/landing page image and extract a complete, hyper-specific style guide to replicate this visual style in future AI image generations.
...
LAYOUT:
- Format and aspect ratio (e.g., 9:16 vertical, 1:1 square)
- Background gradient: direction, start color (hex) → end color (hex), any mid-stop colors
- Exact vertical zone breakdown as percentages (e.g., header zone: top 15%, product zone: 15–55%, benefits zone: 55–75%, footer zone: 75–100%)
- How product and person relate spatially: do they share the same zone, overlap, or occupy completely separate zones?
- Overall compositional flow (e.g., top-to-bottom reading order, central anchor, diagonal tension)

TYPOGRAPHY:
- For EACH text element (main headline, subheadline, badge text, benefit labels, footer text, seal text): font weight (thin/regular/bold/black), letter case (ALL CAPS / Title Case / lowercase), color (hex), estimated size in pt relative to canvas, alignment (left/center/right), and any stroke/outline (color hex, thickness in px)
- Mixed-color text: specify which words are which color
- Any text with glow, shadow, or special effect: describe precisely

GRAPHIC ELEMENTS:
- Badge: exact shape (circle / rounded rectangle / pill / starburst / pentagon), fill color (hex), border if any (color hex, thickness), text content and style
- Product presentation: how it is shown (packshot, in-use, floating, angled), specific visual effect applied (energy burst, radial glow, drop shadow, rim light, color halo — describe color hex and intensity)
- Benefits container: EXACT container type for each benefit — this is critical:
  ...
  * Icon + text inline
  Specify fill color (hex), border color (hex), icon style if present
- Seal/stamp: shape (circle / badge / ribbon), number of color rings and their hex codes, text arranged in arcs (specify exact text for top arc, center, bottom arc)
- Footer bar: background color (hex), content description, text style

COLOR PALETTE:
- List every distinct color with its hex code and role (e.g., "#1A0A2E — deep violet background base")
- All gradients: label (e.g., "background gradient"), start hex → end hex, direction
- Special effect colors: glow hex, burst hex, overlay hex with opacity if visible

MOOD & STYLE:
- 3–5 specific mood adjectives (e.g., "electric, aggressive, clinical, aspirational")
- Photography style (e.g., "studio packshot on gradient", "lifestyle outdoor action", "composite illustration")
- Brand feel with concrete reference (e.g., "GNC / Optimum Nutrition supplement energy aesthetic", "Apple minimalist product launch", "fast-fashion Instagram DTC")

COMPOSITION RULE:
- How the visual zones connect (e.g., "product burst bleeds into benefits zone creating continuity")
- Which element acts as the visual connector between zones
- What is explicitly PROHIBITED when replicating this style (e.g., "NO rectangular cards for benefits — circles only", "NO white background", "NO lifestyle photography — packshot only")
